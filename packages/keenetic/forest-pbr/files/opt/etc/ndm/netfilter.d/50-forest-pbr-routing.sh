...
logger -t "forest-pbr" "Refreshing routing state after netfilter change"
/opt/etc/init.d/S80forest-pbr reapply-firewall >/dev/null 2>&1 || exit 0
...
for _key in net.ipv4.netfilter.ip_conntrack_fastnat net.netfilter.nf_conntrack_fastnat; do
    ...
    if [ "$_val" != "0" ]; then
        ...
        logger -t "forest-pbr" "Re-disabled HW NAT fastnat ($_key was $_val)"
    fi
done
unset _key _val
