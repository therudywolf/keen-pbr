import { Loader2, Radar, Search, ShieldAlert, ShieldCheck } from "lucide-react"
...
export function ServicesPage() {
  ...
  /**
   * Single-row leak check: mark the row loading, probe the service's target via
   * the routing-test mutation, then store the verdict. The bulk/detailed leak
   * experience lives on the dedicated Leak Scanner page; this is just a quick
   * per-row spot check.
   */
  const handleLeakCheck = useCallback(
    async (service: string, list: ListConfig) => {
      ...
      const verdict = await runServiceLeakCheck(target, (probe) =>
        routingTestMutation.mutateAsync({ data: { target: probe } }),
      )

      setLeakChecks((previous) => ({ ...previous, [service]: verdict }))
    },
    [routingTestMutation, t],
  )
  ...
  const tableRows = filteredServices.map(([service, list]) => {
    ...
    return [
      ...
      <LeakCheckCell
        key={`${service}-leak`}
        onCheck={() => void handleLeakCheck(service, list)}
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        actions={
          <Button
            onClick={() => navigate("/leak-scanner")}
            variant="outline"
          >
            <Radar className="mr-1 h-4 w-4" />
            {t("pages.services.openScanner")}
          </Button>
        }
