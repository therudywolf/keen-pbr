...
DnsmasqGenerator::DnsmasqGenerator(const DnsServerRegistry& dns_registry,
    ...
    : dns_registry_(dns_registry),
      ...
      dns_split_enabled_(dns_split_enabled) {}
...
void DnsmasqGenerator::generate_directives(
    ...
    if (hash_record_callback) {
        ...
        // Fold dns_split into the hash so toggling it triggers a dnsmasq restart
        // (the resolver-config hash is what the restart guard compares).
        hash_record_callback(std::string("dns-split|") + (dns_split_enabled_ ? "1" : "0"));
    }
    ...
    // DNS-correlation split routing: only when explicitly enabled, ask dnsmasq
    // to log every reply (log-queries=extra) to a dedicated file the daemon's
    // DnsSplitObserver tails. When disabled, NOTHING is emitted here — dnsmasq
    // logging is left exactly as it was, the strict-no-op guarantee for the
    // default config.
    if (dns_split_enabled_ && out != nullptr) {
        *out << "# DNS-correlation split routing (daemon.dns_split_enabled)\n";
        *out << "log-queries=extra\n";
        *out << "log-facility=" << kDnsSplitLogPath << "\n\n";
    }
